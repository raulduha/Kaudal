...
const nextConfig = {
  ...
  async headers() {
    // Next dev usa eval() para React Fast Refresh. Permitirlo solo en local:
    // el bundle de producción conserva una CSP estricta sin unsafe-eval.
    ...
    return [{
      ...
      headers: [
        ...
        { key: "Content-Security-Policy", value: `default-src 'self'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'; object-src 'none'; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; ${scriptSrc}; connect-src 'self' https://*.supabase.co wss://*.supabase.co;` },
      ],
    }];
  },
};
